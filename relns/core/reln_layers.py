...
class ReLNLinear(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(ReLNLinear, self).__init__()
...
class ReLNLieBracketNoResidualConnect(nn.Module):
    def __init__(self, in_channels, algebra_type='sl3', share_nonlinearity=False):
        super(ReLNLieBracketNoResidualConnect, self).__init__()
        self.share_nonlinearity = share_nonlinearity
        self.algebra_type = algebra_type
...
class ReLNEquivairanChannelMixing(nn.Module):
    def __init__(self, in_channel) -> None:
        super(ReLNEquivairanChannelMixing).__init__()

        self.ln_linear_relu = ReLNLinearAndKillingRelu(in_channel, 3, algebra_type='so3', share_nonlinearity=False),
        self.ln_pooling = ReLNMaxPool(3)
...
class ReLNLinearAndLieBracket(nn.Module):
    def __init__(self, in_channels, out_channels, algebra_type='sl3', share_nonlinearity=False):
        super(ReLNLinearAndLieBracket, self).__init__()
        self.share_nonlinearity = share_nonlinearity

        self.linear = ReLNLinear(in_channels, out_channels)
        self.liebracket = ReLNLieBracket(
...
class ReLNLinearAndLieBracketChannelMix(nn.Module):
    def __init__(self, in_channels, out_channels, algebra_type='so3', share_nonlinearity=False, residual_connect=True):
        super(ReLNLinearAndLieBracketChannelMix, self).__init__()
        self.share_nonlinearity = share_nonlinearity
        self.linear = ReLNLinear(in_channels, out_channels)
        self.liebracket = ReLNLieBracketChannelMix(
...
class ReLNLinearAndLieBracketNoResidualConnect(nn.Module):
    def __init__(self, in_channels, out_channels, algebra_type='sl3', share_nonlinearity=False):
        super(ReLNLinearAndLieBracketNoResidualConnect, self).__init__()
        self.share_nonlinearity = share_nonlinearity

        self.linear = ReLNLinear(in_channels, out_channels)
        self.liebracket = ReLNLieBracketNoResidualConnect(
            out_channels, algebra_type=algebra_type, share_nonlinearity=share_nonlinearity)
...
class ReLNLinearAndKillingReluAndPooling(nn.Module):
    def __init__(self, in_channels, out_channels, algebra_type='sl3', share_nonlinearity=False, abs_killing_form=False,
        ...
        super(ReLNLinearAndKillingReluAndPooling, self).__init__()
        self.share_nonlinearity = share_nonlinearity

        self.linear = ReLNLinear(in_channels, out_channels)
        self.leaky_relu = ReLNKillingRelu(
            out_channels, algebra_type=algebra_type, share_nonlinearity=share_nonlinearity)
        self.max_pool = ReLNMaxPool(
            out_channels, algebra_type=algebra_type, abs_killing_form=abs_killing_form)
        self.use_batch_norm = use_batch_norm

        if use_batch_norm:
            self.batch_norm = ReLNBatchNorm(out_channels, dim=dim)
...
class ReLNBatchNorm(nn.Module):
    def __init__(self, num_features, dim, algebra_type='sl3', affine=False, momentum=0.1):
        super(ReLNBatchNorm, self).__init__()
        self.dim = dim
        if dim == 3 or dim == 4:
...
class ReLNInvariant(nn.Module):
    def __init__(self, in_channel, algebra_type='sl3', dir_dim=8, method='learned_killing'):
        super(ReLNInvariant, self).__init__()

        self.hat_layer = HatLayer(algebra_type=algebra_type)
        self.learned_dir = ReLNLinearAndKillingRelu(
...
class ReLN_SE(nn.Module):
    def __init__(self, in_channels, out_channels, reduction=16, algebra_type='so3'):
        ...
        """
        super(ReLN_SE, self).__init__()
        self.channels = in_channels
        self.reduction = reduction
        self.algebra_type = algebra_type
        
        # Excitation network: reduce F -> F/r -> F
        self.global_pool = ReLNMaxPool(in_channels)
        self.fc1 = ReLNLinear(in_channels, in_channels // reduction)
        self.fc2 = ReLNLinear(in_channels // reduction, in_channels)
        
        # Activations
        self.relu = ReLNKillingRelu(in_channels // reduction, algebra_type=algebra_type, share_nonlinearity=False, leaky_relu=False, negative_slope=0.2)
...
class ReLN_ChannelSpatialAttention(nn.Module):
    def __init__(self, in_channels, reduction_factor=16, spatial_kernel_size=7, algebra_type='so3'):
        super(ReLN_ChannelSpatialAttention, self).__init__()
        
        # Channel Attention (CA) Module
        self.avg_pool = ReLNMeanPool() 
        self.max_pool = ReLNMaxPool(in_channels)
        
        # Fully connected layers for CA (two 1D convolutions)
        self.fc1 = nn.Conv1d(in_channels, in_channels, kernel_size=1, bias=False)
        self.relu = ReLNKillingRelu(in_channels, algebra_type=algebra_type, share_nonlinearity=False, leaky_relu=False, negative_slope=0.2)
...
class ReLNMaxPool(nn.Module):
    ...
    def forward(self, x):
        ...
        x_max = x[index_tuple]
        return x_max
# class ReLNMeanPool(nn.Module):
#     def __init__(self, kernel=2):
#         super(ReLNMeanPool, self).__init__()
#         self.kernel = kernel    
#     def forward(self, x):
#         B,C,N,W = x.shape
