...
def main():
    ...
    if config['model_type'] == "ReLN_relu_bracket":
        model = ReLNReluBracketPlatonicSolidClassifier(3).to(device)
    elif config['model_type'] == "ReLN_relu":
        model = ReLNReluPlatonicSolidClassifier(3).to(device)
    elif config['model_type'] == "ReLN_bracket":
        model = ReLNBracketPlatonicSolidClassifier(3).to(device)
    elif config['model_type'] == "MLP":
        model = MLP(288).to(device)
    elif config['model_type'] == "ReLN_bracket_no_residual":
        model = ReLNBracketNoResidualConnectPlatonicSolidClassifier(3).to(device)
