...
class LNBatchNorm(nn.Module):
    ...
    def __init__(self, num_features: int):
        super().__init__()
        # We will normalize the Frobenius norm, which is a single scalar
        # feature for each of the `num_features` channels.
        self.bn = nn.BatchNorm1d(num_features)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x: A tensor of shape (B, C, 9, N), where B is batch size,
               C is num_features, and N is sequence length.
        
        Returns:
            A tensor of the same shape with batch normalization applied.
        """
        # Get original shape info
        batch_size, num_features, _, num_samples = x.shape

        # 1. Permute to group the matrix dimension at the end: (B, C, 9, N) -> (B, C, N, 9)
        x_permuted = x.permute(0, 1, 3, 2)

        # 2. Reshape into matrices: (B, C, N, 9) -> (B, C, N, 3, 3)
        matrix = x_permuted.reshape(batch_size, num_features, num_samples, 3, 3)

        # 3. Calculate the rotation-invariant magnitude (Frobenius norm)
        # The norm will have shape (B, C, N)
        norm = torch.norm(matrix, p='fro', dim=(-2, -1))

        # 4. Apply standard BatchNorm1d to the invariant magnitude
        # BatchNorm1d expects (B, C, L), which matches our norm's shape
        norm_bn = self.bn(norm)

        # 5. Re-scale the original matrix
        # Unsqueeze norms for broadcasting: (B, C, N) -> (B, C, N, 1, 1)
        norm = norm.unsqueeze(-1).unsqueeze(-1)
        norm_bn = norm_bn.unsqueeze(-1).unsqueeze(-1)

        # Apply the scaling
        normalized_matrix = matrix / (norm + EPS) * norm_bn
        
        # 6. Reshape and permute back to the original format
        # (B, C, N, 3, 3) -> (B, C, N, 9)
        output_permuted = normalized_matrix.reshape(batch_size, num_features, num_samples, 9)
        # (B, C, N, 9) -> (B, C, 9, N)
