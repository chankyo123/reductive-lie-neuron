...
class SequencesDataset:
    ...
    def unpack_data_window(self, seq_data, seq_desc, row):
        ...
        for i, sensor_name in enumerate(self.sensor_file_basenames):
            ...
            # 1. 입력(Feature)을 월드 좌표계 속도(velocity)로 설정합니다.
            # velocity는 14, 15, 16번째 컬럼에 해당합니다.
            # 기존 IMU(accel, gyro) 대신 velocity를 사용합니다.
            ...
            # feats 딕셔너리에 'velocity'라는 키로 저장합니다.
            feats = {
            ...
            # 2. 목표(Target) 계산을 위해 위치(position)와 회전(rotation) 데이터를 추출합니다.
            # rotation(quat)은 7-10, position은 11-13번째 컬럼에 해당합니다.
            rot = data_chunk[:, 7:11]
            pos = data_chunk[:, 11:14]
            # vel은 이제 입력이므로 GT에서는 필요하지 않을 수 있습니다.
